"""
Code for the computation of kucherenko indices (mostly) taken from
https://github.com/aleeciu/GSA_dependent_variables except function
``kuchernko_indices``.
"""
import numpy as np
import pandas as pd
...
def kucherenko_indices(func, covariance, mean, problem, N):
    ...
    Returns:
        first_order (float): First order index.
        total (float): Total index.

    """
    x, xc = kucherenko_sampling(problem, N, covariance, mean)
    first_order, total = sobol_indexes(func, x, xc, problem)
...
def to_marginal(x, dist, prms):
    ...
    if dist == norm:
        ...
        x = pd.DataFrame(dist.ppf(norm.cdf(x), loc=loc, scale=scale), columns=x.columns)
        ...
        x = pd.DataFrame(
            dist.ppf(norm.cdf(x), shp1, shp2, loc=loc, scale=scale), columns=x.columns
        )
        ...
        x = pd.DataFrame(
            dist.ppf(norm.cdf(x), shp1, loc=loc, scale=scale), columns=x.columns
        )
    else:
